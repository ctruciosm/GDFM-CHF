function [H,univariate] = dcc_fit_variance_fore(data,p,o,q,gjrType,startingVals)
...
[T,k] = size(data);
H = zeros(T+1,k);                                                           %T changed by T+1
univariate = cell(k,1);
...
for i=1:k
    ...
    [parameters, ~, ht, ~, ~, scores, diagnostics] = tarch_fore(data(:,i),p(i),o(i),q(i), [], gjrType(i), volStartingVals, univariteOptions);

...
/*
 * vt_likestream_core.c -
 * Likelihood for estimation of variance targeting scalar BEKK(1,0,1) multivarate volatility
 * model with GARCH(1,1) conditional variances
 *
 * Based on scalar_vt_vech_likelihood.m but try to make it run faster
 *
 * To be used in the call to fmincon optimizer in vtstreamXX.m
 */

void vt_likestream_core(double a, double b, const size_t k, const size_t T, double *stdData, double *C, double *backCast, double *llsum) {
   
   /* int count = 0; */
   /* mexPrintf("vt_likestream_core marker %d \n",count); */
   /* 0 */
   /* count++; */
   double *intercept, aplusb, omega, *S, *X, *indices;
   double *ll;
   ...
   *llsum  = 0.0;
   ...
   for (t=0;t<T;t++)
   {
      ...
      if (t==0)
      {
         S[0] = intercept[0] + (aplusb*backCast[0]);
         for (i=1;i<k;i++) 
         {
            for (j=i-1;j<=i;j++)
            {
               S[i+(k*j)] = intercept[i+(k*j)] + (aplusb*backCast[i+(k*j)]);
            }
         }
      }
      else
      {
         S[0]=intercept[0] + (a*stdData[(elements*(t-1))]) + (b*S[0]);
         for (i=1;i<k;i++) 
         {
            for (j=i-1;j<=i;j++)
            {
               S[i+(k*j)]=intercept[i+(k*j)] + (a*stdData[i+(k*j)+(elements*(t-1))]) + (b*S[i+(k*j)]);
            }
         }
      }
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
   ...
   double *parameters, *stdData, *C, *backCast, a, b;
   int j;
   size_t k, T;
   ...
   vt_likestream_core(a,b,k,T,stdData,C,backCast,llsum);
   ...
   if (1==2) {
      ...
      printf("vt_likestream in C took %f seconds to execute \n", time_taken);
   }   
}
